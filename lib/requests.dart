...
Future<bool> getAreaState(String areaId) async {
  ...
  String key = decoded.containsKey('access_doors') ? 'access_doors' : 'areas';
  if (areaId == 'building')
    {
      List<dynamic> areas = decoded['areas'];

      return true;
    } else {
    switch (key) {
      case 'access_doors':
        dynamic doorValue = decoded['access_doors'][0]['state'];
        if (doorValue == 'locked')
          return true;
        else
          return false;
      case 'areas':
        List<dynamic> areas = decoded['areas']; // Assuming similar structure
        for (var area in areas) {
          if (area.containsKey('access_doors')) {
            dynamic doorValue = area['access_doors'][0]['state'];
            if (doorValue != 'locked') {
              return false;
            }
          }
        ...
        return true;
      default:
        throw Exception('Key not handled');
    }
  }
}
...
// credential 11343 corresponds to user Ana of Administrator group
  ...
  final String responseBody = await sendRequest(uri);
  //print('requests.dart : door ${area.id} is ${door.state}');
}
